...
A continuaci\'on presentamos los cinco principio que (\cite{essenceofalgol})
...
Stack discipline tiene que ver con el compactado del ambiente y el estado al
momento de que identificador de variable pierde se pierde del alcance.
Sobre el \'ultimo punto vamos a presentar una variante de sem\'antica para el lenguaje
...
La modificaci\'on de los tipos nos obliga a actualizar la relaci\'on de
...
Sea $\Sigma_\alpha$ $=$ $\{$ $\sigma$ | tal que $\sigma$ tiene forma $\alpha$ $\}$, luego\\

\indent
$\C_0$ $=$ $\{$ $\Sigma_\alpha$ | $\alpha$ es una forma $\}$\\
\indent
$\C_1(\Sigma_\alpha,\Sigma_{\alpha'})$ $=$ $\{(h,s)$ | si $\alpha'$ extiende a $\alpha$ $\}$\\
...
De esta definici\'on la ecuaci\'on que m\'as nos va a interesar ser\'a $\semBrcks{\pi}_0(h,s) \ \eta$,
...
En este punto vamos a hacer un par\'entesis para comentar la restricci\'on
...
como antes, tenemos que ver qui\'enes ser\'an $\eta_{new}$ y $\sigma_{new}$; empezando por
...
sin stack discipline. Lo que pretendemos con esta sem\'antica ser\'ia, 
...
Notar que no definimos que pasa para el caso particular de tener $f_{\delta}(\iota_{\delta'}x)$
con $\delta \neq \delta'$, 
...
Para solucionar esto podemos pensar en qu\'e casos nuestra sem\'antica sin
